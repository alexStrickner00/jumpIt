static class CollisionHelper {
  ...
  public static boolean collide(Player p, Platform pf) {
    ...
    if (((((p.y - p.sprite.height) <= pf.y) && (p.y - p.sprite.height) >= (pf.y-pf.H)) || (p.y - p.sprite.height >= pf.y && p.y - p.sprite.height + p.getVy() * getSpeedMultiplier() < pf.y - pf.H)) && !(p.x > (pf.x + pf.W)) && !((p.x + p.sprite.width) < pf.x)) {
      return true;
    }
    return false;
  }
